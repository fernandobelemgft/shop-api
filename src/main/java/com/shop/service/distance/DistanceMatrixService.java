...
public class DistanceMatrixService {
	...
	public DistanceMatrixService(@Value("${api.matrix.key}") String key) {
		this.key = key;
	}

	/**
	 * Returns the nearest {@link Shop} according with 
	 * provided origin geolocation parameters
	 * @param shops
	 * @param originLat
	 * @param originLgt
	 * @return
	 * @throws LocationNotFoundException when the given information 
	 * is not valid to acquire users location
	 */
	public Shop getNearest(List<Shop> shops, String originLat,
			String originLgt) throws LocationNotFoundException {

		// Map created to attend the necessity of wrapping the Shop object 
		// and the DistanceElement that will be provided by the distance matrix api
		Map<Shop, DistanceElement> map = new HashMap<Shop, DistanceElement>();
		
		if(originLat.isEmpty() || originLgt.isEmpty()){
			throw new LocationNotFoundException("Device location not detected, you may want to check your location service");
		}

			
		//for each shop in database a DistanceElement will be retrieved informing the 
		...
		for (Shop shop : shops) {
			...
			String formmatedURL = String.format(DISTANCE_MATRIX_URL, originLat,
					originLgt, shop.getShopAddress().getLatitude(), shop.getShopAddress().getLongitude(), key);
			
			DistanceDetailsResponse response = new RestTemplate().getForObject(
					formmatedURL, DistanceDetailsResponse.class);
			
			if (response.getDistanceDetails()[0].getDistanceElements()[0].getDistance() == null) {
				...
				map.put(shop, response.getDistanceDetails()[0].getDistanceElements()[0]);
			}
	...
	/**
	 * A simple method designed to extract the 
	 * searching functionality of finding the option containing 
	 * the smallest distance value
	 * @param map created to wrap {@link Shop} and {@link DistanceElement}
	 * @return
	 */
